...
const ITEMS = {
  ...
  segredo:[
    ...
    {n:'Vanguarda de Eclipse', t:'faca', i:'🗡️'},
    {n:'Barrett do Crepúsculo', t:'arma', i:'🔫'},
    {n:'Runas de Carnificação AK', t:'arma', i:'🔫'},
    {n:'M16 Polarizado Escarlate', t:'arma', i:'🔫'},
    {n:'Cicatriz do Crepúsculo', t:'arma', i:'🔫'},
    {n:'Blaster de Hipercarga Rosa', t:'arma', i:'🔫'}
  ],
  especial:[
...
const SPECIAL_ITEMS = {
  ...
  dragon:    {name:'Transformação do Dragão', icon:'🐉', theme:'Drop de Raid — Kaido', desc:'Especial: por 30 segundos, sua vida e seu escudo se igualam aos do inimigo atual. Reutilizável, com cooldown de 15 segundos, como os demais especiais.'}
};
...
function normalizeItem(it){
  let category = it.category||'weapon';
  if(it.key==='dragon') category = 'special'; // migra saves antigos: Dragão agora é equipável, não consumível
  return Object.assign({
    id:it.id, category:category, rarity:it.rarity||null,
...
function openItemModal(id){
  ...
  const starsHtml = ('★'.repeat(item.stars||0)+'☆'.repeat(5-(item.stars||0)));
  ...
  overlay.innerHTML =
    '<div class="modal">'+
      ...
      '<div class="modal-passive">'+(item.passive? 'Passiva: '+PASSIVES[item.passive].label+' (+'+PASSIVES[item.passive].bonus+'x)' : 'Sem passiva')+'</div>'+
      descHtml+
...
document.getElementById('admAddSpecial').addEventListener('click', ()=>{
  ...
  addToInventory(item);
  renderAdminInvList();
  toast('🛠️ Item especial adicionado ao inventário: '+item.name);
});
...
function renderSpecialButtons(){
  const esp1 = getSpecial1Item();
  const esp2 = getSpecial2Item();
  document.getElementById('specialLeftIcon').textContent = esp1? esp1.icon : '✖';
  document.getElementById('specialRightIcon').textContent = esp2? esp2.icon : '✖';
...
function startMarker(){
  ...
  function step(ts){
    ...
    try{
      ...
      console.error('battle tick error (ignorado para não travar a barra):', err);
    }
...
function useSpecial(slot){
  ...
  switch(item.key){
    ...
    case 'kokushibo':
      ...
      break;
    case 'dragon':
      state.player.hp = Math.min(state.player.maxHp, battle.hp);
      state.player.shield = battle.shield;
      state.player.dragonUntil = now+30000;
      toast('🐉 Transformação do Dragão: sua vida e escudo se igualaram aos do inimigo por 30s!');
      break;
    default: used=false;
  }
  if(!used) return;
...
function resolveEnemyDefeat(){
  ...
  if(battle.mode==='kaido'){
    ...
    if(Math.random() < DRAGON_DROP_CHANCE){
      const dragonItem = normalizeItem({id:uid(), category:'special', rarity:null, name:SPECIAL_ITEMS.dragon.name, type:null, key:'dragon', icon:SPECIAL_ITEMS.dragon.icon, stars:0, passive:null, equipped:false});
